...
describe("NFT auction Tests For V1", function () {
  ...
  beforeEach(async function () {
    ...
    nftAuctionByFirstAccount = await ethers.getContract(
      "NFTAuctionV1",
      firstAccount
    );
    nftAuctionBySecondAccount = await ethers.getContract(
      "NFTAuctionV1",
      secondAccount
    );
    nftAuctionByThirdAccount = await ethers.getContract(
      "NFTAuctionV1",
      thirdAccount
    );
    nftAuctionByFourthAccount = await ethers.getContract(
      "NFTAuctionV1",
      fourthAccount
    );
  ...
  it("Test function publish: revert with message NFT is already published", async function () {
    ...
    await nftAuctionBySecondAccount.publish(
      mockNFTSecondAccount,
      tokenId,
      1,
      3600 * 1000
    );
    await expect(
      nftAuctionBySecondAccount.publish(
        mockNFTSecondAccount,
        tokenId,
        1,
        3600 * 1000
      )
    ).to.be.revertedWith("Auction is already published");
  });
  ...
  it("Test function publish: successfully called", async function () {
    ...
    expect(await nftAuctionBySecondAccount.nftAddress()).to.be.eq(
      mockNFTSecondAccount
    );
    expect(await nftAuctionBySecondAccount.price()).to.be.eq(
      ethers.parseEther("1")
    );
    expect(await nftAuctionBySecondAccount.bidPrice()).to.be.eq(
      ethers.parseEther("1")
    );
  });
  ...
  it("Test function bidByETH: revert with message Auction is not published Or End", async function () {
    ...
    await expect(
      nftAuctionByThirdAccount.bidByETH({ value: ethers.parseEther("0.1") })
    ).to.be.revertedWith("Auction is not published Or End");
  });
  ...
  it("Test function bidByETH: revert with message Bid price must be greater than current bid price", async function () {
    ...
    await expect(
      nftAuctionByThirdAccount.bidByETH({ value: ethers.parseEther("0.1") })
    ).to.be.revertedWith("Bid price must be greater than current bid price");
  });
  ...
  it("Test function bidByETH: transfer ETH to account with lower bid price", async function () {
    ...
    expect(
      await ethers.provider.getBalance(nftAuctionByThirdAccount)
    ).to.be.equal(ethers.parseEther("11"));

    await nftAuctionByFourthAccount.bidByETH({
      value: ethers.parseEther("13"),
    });
    expect(
      await ethers.provider.getBalance(nftAuctionByThirdAccount)
    ).to.be.equal(ethers.parseEther("13"));
  });
  ...
  it("Test function endAuction: revert with message Only auction owner can operation", async function () {
    ...
    await expect(nftAuctionByThirdAccount.endAuction()).to.be.revertedWith(
      "Only auction owner can operation"
    );
  ...
  it("Test function endAuction: successfully called", async function () {
    ...
    await mockNFTSecondAccount.mintNFT(ipfsUrl);
    expect(
      await mockNFTSecondAccount.ownerOf(await mockNFTSecondAccount.tokenId())
    ).to.be.eq(secondAccount);

    await mockNFTSecondAccount.approve(
      nftAuctionBySecondAccount,
      await mockNFTSecondAccount.tokenId()
    );
    await nftAuctionBySecondAccount.publish(
      mockNFTSecondAccount,
      await mockNFTSecondAccount.tokenId(),
    ...
    expect(
      await mockNFTSecondAccount.ownerOf(await mockNFTSecondAccount.tokenId())
    ).to.be.eq(nftAuctionByFirstAccount);
    ...
    expect(
      await ethers.provider.getBalance(nftAuctionByFirstAccount)
    ).to.be.equal(ethers.parseEther("13"));
